...
# === SBATCH Directives (Modified for Learning Rate Sweep) ===
#SBATCH --job-name=gpt2_p6000_sif_10m_LR_SWEEP # Modified job name
...
#SBATCH --array=1-5 # SBATCH array for 5 learning rates
...
# --- Define Run Output Directory Name (Modified for LR sweep) ---
BASE_RUN_OUTPUT_NAME="gpt2_p6000_sif_local_eval_run_May_LR_sweep"
...
# --- Preparations (Unchanged, but output dir creation is now per LR) ---
echo "Project Directory (Host): ${HOST_PROJECT_DIR}"
echo "SIF Image Path (Host): ${HOST_SIF_PATH}"
# ... (rest of preparations are the same)
...
# === Training Script Execution (Modified for LR Sweep) ===
echo "Starting Python training script for LR ${CURRENT_LR} (Name: ${CURRENT_LR_NAME}) inside Singularity container..."
...
# --- Define Neptune args (Modified for LR sweep) ---
NEPTUNE_PROJECT_ARG=""
...
NEPTUNE_RUN_NAME="${RUN_OUTPUT_NAME}_$(date +%Y%m%d_%H%M)" # Neptune run name now includes LR
NEPTUNE_TAGS="p6000 test1 baseline singularity py39 local_eval lr_sweep lr_${CURRENT_LR_NAME}" # Added LR-specific tag

# --- Set PyTorch CUDA Allocator Config (Unchanged) ---
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True

# --- Execute Singularity Command (Modified for LR sweep) ---
singularity exec --nv \
